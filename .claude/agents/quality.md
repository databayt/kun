...
> Code-side keywords cite the atomic rules under `.claude/rules/<domain>/` (frontmatter: `domain`, `severity`, `paths`, `since`; Good/Bad/Fix body). Report each finding as `rule-id (severity)` — e.g. `use-action-state (error)`. Full mapping: `.claude/rules-global/patterns.md` § Rule Corpus.
