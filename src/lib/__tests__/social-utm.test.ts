...
const ctx = { channel: "telegram", brand: "hogwarts" };
...
describe("applyUtm", () => {
  it("tags a bare link with source, medium and campaign", () => {
    ...
    expect(p.get("utm_source")).toBe("telegram");
    expect(p.get("utm_medium")).toBe("social");
    expect(p.get("utm_campaign")).toBe("hogwarts");
  });

  it("separates channels, which is the whole point", () => {
    const tg = applyUtm("https://databayt.org/x", ctx);
    const fb = applyUtm("https://databayt.org/x", {
      ...ctx,
      channel: "facebook",
    });
    expect(tg).not.toBe(fb);
  ...
  it("tags every link in a multi-link post", () => {
    ...
    expect(out.match(/utm_source=telegram/g)).toHaveLength(2);
  });
