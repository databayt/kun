...
export const schools: School[] = [
  ...
  },
  {
    ...
    spells: [
      ...
      },
      {
        name: "deploy",
        effect:
          "Production deploy, the whole cycle: gate, push main, env, schema gap, worktree build, smoke, deploy, Neon restore point, prod data steps, real login",
        order: [
          f("deploy"),
          f("build"),
          p("Cloudflare"),
        ...
        steps: [
          "ls wrangler.jsonc → Cloudflare lane (hogwarts, mkan); otherwise the legacy Vercel lane",
          "tsc with an 8 GB heap; kill next dev; commit tracked files; pull --rebase and push main",
          "vercel env pull /tmp/prod.env (env storage still works; deploys do not)",
          "prisma migrate diff against prod: added columns block, index drift is noted",
          "CF_SOURCE=worktree deploy-cloudflare.sh build → smoke (read the curl table) → deploy (note the version id)",
          "Neon restore point as a no_compute branch; at the free-tier limit delete the OLDEST branch and retry",
          "Run the seeds and scripts the deploy never runs against the prod DIRECT_URL",
          "Real login on the live hostname, then the memory line: Worker id, main sha, restore branch",
          "Prune restore-point-* branches older than 7 days",
        ],
        connects: ["build", "ship", "push", "cloudflare"],
        depends: ["build"],
      },
      {
        name: "release",
  ...
  },
  {
    ...
    spells: [
      ...
      },
      {
        name: "report",
        effect:
          "List the open user reports, let a human take or reject each, fix the accepted ones",
        order: [
          f("report"),
          f("quality"),
          f("sse"),
        ...
        steps: [
          "Session start: the hook lists the open report queue across all repos (one search call) — it never fixes",
          "LIST: gh search issues --owner databayt --label report --state open → numbered table (accepted → verified → team → needs-human → low-confidence → legacy)",
          "CHOOSE: Abdout takes / rejects in prose (AskUserQuestion only for ≤4 rows, never in an autonomous session)",
          "APPLY: taken → `accepted` label + comment; rejected → closed as not planned with the reason",
          "READ: gh issue view → page URL + description + score-block (team, bucket, scores)",
          ...
          "VALIDATE: accepted still has to be a bug the QA scope allows — data gaps and feature requests go back with a comment",
          "SEE + DEBUG: screenshot page, check console errors, network failures",
          "FIX: edit minimum code in component directory",
          "BUILD: pnpm tsc --noEmit + pnpm build — verify no regressions",
          "PUSH: conventional commit (fix: … Closes #N) straight to main",
          "VERIFY: kun redeploys on push; hogwarts/mkan need /deploy (Cloudflare)",
          "CLOSE: close issue with fix summary and which platform has it",
        ],
  ...
  },
  {
    ...
    spells: [
      ...
      },
      {
        name: "facebook",
        effect:
          "Connect a brand's Facebook Page to the pipeline — grant, permanent token, registration, a verified-then-retracted first post",
        order: [f("growth"), s("/facebook")],
        steps: [
          "The Page id is NOT the number in profile.php?id= — that is the profile number and Graph rejects it (error_subcode 33); take the real id from the Page picker or /me/accounts",
          "App must be Business type — a Consumer app can never request Pages scopes ('Invalid Scopes') and the type cannot be changed after creation",
          "If the app already holds other Pages, Generate Access Token SILENTLY re-issues the old grant — click Edit settings → Select all → Continue → Save, then Generate",
          "Never revoke the app to force a fresh dialog: it invalidates every other brand's permanent Page token",
          "Long-lived exchange needs no app secret — the Access Token Debugger's 'Extend Access Token' does it, then /me/accounts derives the Page token",
          "Require type: PAGE and expires_at: 0 before storing anything; a token that expires is a scheduled outage",
          "/me/accounts returns only personally-owned Pages — a Business-Portfolio Page is granted and postable but absent there",
          "Register in four files (products.ts, carousel schema.ts, brands.ts, docs meta.json + <brand>.mdx); transport code never changes, env resolves by suffix",
          "Env FACEBOOK_PAGE_ID_<BRAND> / FACEBOOK_PAGE_ACCESS_TOKEN_<BRAND> in .env, .env.example, and Vercel production+preview+development",
          "Prove with a real post (health check alone is insufficient), read it back for a live permalink, then retract it — and re-check every sibling brand, since a grant change is when their tokens break",
        ],
        connects: ["social", "publish", "draft", "approve", "measure"],
        depends: [],
      },
    ],
  },
    ...
    spells: [
      ...
      },
      {
        ...
        steps: [
          ...
          "The recurring calendar IS content/social/pillars.json — the Monday seeder rotates it into the draft queue, and /social/calendar renders it live: this week's picks highlighted, a queue chip per brief, Queue-now files an ask on the spot",
      ...
      },
      {
        name: "draft",
        effect:
          "Write the FULL draft — copy (Arabic crafted first, English mirrored, UTM on every link) and/or its media set",
        ...
        steps: [
          ...
          "Settle the media half: pick from library.json (the (library: id) hint wins), generate via /higgs or /carousel when nothing matches, or say text-only — queue mode passes cdnUrls with answer --media, never an invented URL",
          "UTM every link as utm_source=<channel>&utm_medium=social&utm_campaign=<brand>",
          "Run the moral gate — no invented metric, customer, price, or date; the answered full draft lands in the review queue on /social/publish",
        ],
        connects: [
          "social",
          "calendar",
      ...
      },
      {
        ...
        steps: [
          ...
          "Register in the library (higgs-library.mjs add … push), then attach the cdnUrl to the draft ask it answers: social-drafts.mjs attach <id> --media",
        ],
        connects: ["social", "draft", "media", "approve", "carousel"],
        depends: [],
      },
      {
        name: "media",
        effect:
          "The draft's media half — pick from the library or generate by lane, then attach to the draft; the showroom's Attach is the same move in the browser",
        order: [m("content/media/library.json"), s("/higgs")],
        steps: [
          "Pick before you generate: match library.json by the brief's (library: id) hint, else brand + assetType — a cdnUrl already paid for is free",
          "Generate only when nothing matches: text-free photography via /higgs, text-bearing formats via /carousel as a 1-slide deck",
          'Register the render (higgs-library.mjs add … push), then attach: node scripts/social-drafts.mjs attach <id> --media "url1,url2" — REPLACES the set',
          "Shapes delivery accepts: images and/or ONE video, max 10 — mixed image+video is refused by name before anything is sent",
          "In the Hub: the showroom's Attach fills the publish tray, and the review queue shows the full draft — copy beside its media — for the human yes",
        ],
        connects: ["draft", "showroom", "publish", "higgs", "carousel"],
        depends: [],
      },
      {
        name: "approve",
        effect:
          "The human gate — the review queue on /social/publish approves in-app, or a signed one-click link reaches a remote approver; either way it ends by stopping",
        ...
        steps: [
          ...
          "Primary lane — the review queue on /social/publish: load the next answered full draft, fine-tune, Approve (claims answered → consumed, then publishes now or schedules to the drain) or Dismiss",
          "Remote lane — stageForReview: one pending variant + one 12h signed link per channel (never /api/social/relay: it publishes immediately); staging retires the queue entry so no draft is approved twice",
          "STOP. Deliver the links and wait; the gate never times out into a publish",
          "Record the decision per channel — approved, held, dismissed, or rejected",
        ],
        connects: ["draft", "media", "higgs", "publish", "carousel"],
        depends: [],
      },
      {
        name: "publish",
        effect:
          "The review queue decides, the lanes deliver — approved full drafts drained where an API exists, copy-out where none does",
        ...
        connects: ["approve", "media", "measure", "social", "calendar"],
        depends: ["approve"],
      },
      {
        name: "measure",
      ...
      },
      {
        ...
        steps: [
          ...
          "Stage 1 /calendar — pick the slot from pillars.json (skipped when the topic arrived as an argument)",
          "Stage 2 /draft — the FULL draft: Arabic-first core piece, platform-native variants, and its media set",
          "Stage 3 media — pick from library.json first; generate only on a miss (/higgs text-free, /carousel text-bearing), then attach; a failure downgrades to a text post, it does not stop the chain",
          "Stage 4 /approve — HARD STOP. The review queue on /social/publish is the primary gate; a signed link covers a remote approver. No human yes, no publish, ever",
          "Stage 5 /publish — drain lane (now or scheduled), hermes lane, or a copy-out block for whatsapp",
      ...
      },
      {
        name: "carousel",
        effect:
          "Multi-slide bilingual brand carousels — Claude writes the deck, the kun render route draws Anthropic-styled slides, Playwright captures exact platform sizes, a human approves before any channel sees it",
        order: [f("growth"), s("/carousel"), p("Figma")],
        steps: [
          "Resolve brand + topic from content/docs/social/<brand>.mdx and brand.mdx (block-scoped decks read the block records)",
          "Write the bilingual deck (AR crafted first, never translated) into content/carousels/<brand>/<slug>.json — hook cover, one idea per slide, CTA close",
          "Render: pnpm carousel:render <brand>/<slug> — PNGs per language and size, PDF variant, per-channel captions with UTM",
      ...
      },
      {
        ...
        steps: [
          ...
          "File every asset: record.sh file <src> --repo --block --url — lands as <repo>--<block>--<url-slug>--<kind>--<locale>--v<N> (designation rides in the filename) with the repo sha in the manifest",
          "Flows ship as ADS: record.sh adcut renders 15-45s (body sped to target, last seconds real-time); set-dress every on-camera page first and zoom the browser to 125%",
    ...
    spells: [
      ...
      },
      {
        ...
        steps: [
          ...
          "Attach fills the publish tray: the asset rides to /social/publish and becomes the draft's media half — no copy-pasting URLs",
        ],
        connects: ["higgs", "carousel", "media", "draft", "measure"],
        depends: [],
      },
    ],
  },
  {
    ...
    number: "20",
    name: "The Owlery",
    subtitle: "Lead acquisition and outreach",
    description:
      "Hogwarts' owls carry post out and bring the reply back — which is the whole job here, and the reason this school is named for delivery rather than for discovery. Measured 2026-08-17, the CRM's 3,156 school leads break down as 176 contactable, 13 with a Facebook page, 32 with a website, and 2,935 that are an OpenStreetMap name and a map pin with no online handle at all. So automated enrichment caps at +45 rows and a full scrape run adds +15, while 131 tier-A/B schools are contactable and unworked today. Every spell here makes contact-gap analysis, enrichment and outreach the easy path and raw discovery a deliberate choice someone made out loud. Arabic forms are first-class — half this work is Arabic. Code lives in hogwarts/scripts/crm/ and mkan/scripts/crm/ (canonical); kun holds the routing and none of the scrapers.",
    quote:
      '"scrap facebook for hogwarts" — and the owl comes back with thirteen pages, not three thousand. Knowing that before you send it is the spell.',
    spells: [
      {
        name: "scrap",
        effect:
          "The umbrella — routes by argument, and with none it measures the contact gap first, because discovery is the low-yield lane here",
        order: [f("lead"), s("/scrape"), h("scrape-guard")],
        steps: [
          "No argument: run contact-gap.ts (read-only) and report the four lanes",
          "Name the yield of each option before choosing one — enrichment caps at +40, discovery adds +15",
          "Recommend the highest-yield move, which is almost always outreach on the 130 already contactable",
          "Dry-run first, --apply second; never write to Twenty by SQL",
        ],
        connects: ["contact gap", "leads", "enrich", "outreach"],
        depends: [],
      },
      {
        name: "scrape",
        effect:
          "The other spelling — same lane as `scrap`, which is how Abdout types it; both route here",
        order: [f("lead"), s("/scrape")],
        steps: ["Identical to `scrap` — see its steps"],
        connects: ["scrap"],
        depends: [],
      },
      {
        ...
        steps: [
          ...
          "npx tsx scripts/crm/contact-gap.ts — read-only by design, it never writes",
          "Flag drift from the 2026-08-17 snapshot rather than quietly reusing it",
        ],
        connects: ["leads", "enrich", "pipeline health"],
        depends: [],
      },
      {
        name: "leads",
        effect:
          "The asset, not the backlog — 130 tier-A/B schools that are contactable and unworked right now",
        order: [f("lead"), s("/scrape"), m("reference_crm")],
        steps: [
          "Filter contactable AND (COLD|PROSPECT) AND tier A|B — that is the workable set",
          "Working it needs no scraping and no enrichment to start",
          "Hand to outreach drafting; personalization comes from Twenty, never from a scraped file",
        ],
        connects: ["prospect", "outreach", "contact gap"],
        depends: [],
      },
      {
        name: "prospect",
        effect:
          "Work one named school — read its tier, stage and next action from the CRM, then draft the move",
        order: [f("lead"), s("/scrape"), m("reference_crm")],
        steps: [
          "Look the company up over REST; never psql into a workspace schema",
          "Check nextActionAt — the digest surfaces overdue actions the pipeline forgot",
          "Log the touch as a Twenty Note so the next session sees it",
        ],
      ...
      },
      {
        name: "enrich",
        effect:
          "Turn a signal into a contact — FB About tab plus website extract. The ceiling is +40 rows, measured, not hoped",
        order: [
          f("lead"),
          s("/scrape"),
          p("Chrome DevTools"),
        ...
        steps: [
          "Only the FB_PAGE (13) and WEBSITE (27) lanes have anything to work on",
          "MAP_ONLY rows carry no online handle — spend nothing on them; they need a directory or a human",
          "Normalize to E.164 and LABEL mobile vs landline — only 45 of 175 are mobile",
          "Checkpoint so a stop is resumable, and log what the stop dropped",
        ],
        connects: ["contact gap", "scrap facebook", "outreach"],
        depends: [],
      },
      {
        name: "outreach",
        effect:
          "Draft the first touch — email leads for the Gulf/Egypt majority (119 emails), WhatsApp for the Sudan slice (45 mobiles)",
        order: [f("lead"), s("/scrape")],
        steps: [
          "Arabic written first, English mirrored; personalization read from Twenty",
          "Email through Resend on an outreach subdomain; WhatsApp through hogwarts' Evolution API",
          "Never build a second sender — hogwarts already ships retry, rate limiter, templates, dispatch",
          "A human approves before anything reaches a school; stop the cadence on any reply",
        ],
        connects: ["leads", "scrap whatsapp", "prospect"],
        depends: [],
      },
      {
        name: "pipeline health",
        effect:
          "The CRM funnel's shape — stages, tiers, overdue next-actions. Not /health (engine config) and not /measure (published-social numbers)",
        order: [f("lead"), s("/scrape"), m("reference_crm")],
        steps: [
          "Read stage + tier + nextActionAt tallies over REST",
          "Surface anything overdue — the King Fahad conversion sat overdue since 2026-06-15 unnoticed",
          "Refresh .claude/memory/pipeline.json from this rather than hand-copying it",
        ],
        connects: ["contact gap", "leads", "captain"],
        depends: [],
      },
      {
        name: "scrap facebook",
        effect:
          "The Page About/Intro lane — 13 rows, the whole surface. Dedicated account, throttled, never Abdout's",
        order: [
          f("lead"),
          s("/scrape"),
          p("Chrome DevTools"),
        ...
        steps: [
          "Read the About/Intro tab, NOT the feed — the feed is why the earlier yield was near zero",
          "Split Pages from Groups; a /groups/ URL is not a school lead",
          "FB_SCRAPE_PROFILE + FB_SCRAPE_PORT + FB_SCRAPE_DELAY_MS, or scrape-guard blocks the run",
          "Publishing moved to a System User (2026-08-19) so a scrape ban no longer touches it — keep the scrape account at ZERO roles so losing it stays cheap",
        ],
        connects: ["enrich", "scrap", "scrap whatsapp"],
        depends: [],
      },
      {
        name: "scrap whatsapp",
        effect:
          "Not a thing — WhatsApp has no scrapeable surface. Routes to what is real: harvest numbers from the public web and FB About tabs, then SEND through the Evolution API",
        order: [f("lead"), s("/scrape")],
        steps: [
          "Say the honest part first: there is no WhatsApp directory to crawl",
          "Harvest: the FB About and website lanes, then normalize-contacts.ts to E.164",
          "Label mobile vs landline — a landline in a WhatsApp campaign is a silent non-delivery",
          "Send: hogwarts/src/lib/whatsapp/ — dedicated number, separate instance, warm-up ramp, stop-on-reply",
        ],
        connects: ["outreach", "scrap facebook", "enrich"],
        depends: [],
      },
      {
        ...
        steps: [
          ...
          "The lead object is the school (Company with stage/tier/leadStatus custom fields)",
        ],
        connects: ["scrap", "leads"],
        depends: [],
      },
      {
        name: "scrap for mkan",
        ...
        steps: [
          ...
          "Same conclusion as hogwarts: coverage is not the constraint, inventory is",
        ],
        connects: ["scrap", "leads"],
        depends: [],
      },
      {
        name: "اسحب",
        effect: "Arabic for `scrap` — the same umbrella, measured first",
        order: [f("lead"), s("/scrape")],
        steps: ["Identical to `scrap` — see its steps"],
        connects: ["scrap"],
        depends: [],
      },
      {
        name: "جمع بيانات",
        effect:
          "Arabic for data collection — routes to enrichment, since the gap is contacts and not names",
        order: [f("lead"), s("/scrape")],
        steps: ["Identical to `enrich` — see its steps"],
        connects: ["enrich", "scrap"],
        depends: [],
      },
      {
        name: "عملاء محتملين",
        effect:
          "Arabic for leads — the 130 contactable tier-A/B schools waiting to be worked",
        order: [f("lead"), s("/scrape")],
        steps: ["Identical to `leads` — see its steps"],
        connects: ["leads", "outreach"],
        depends: [],
      },
    ],
  },
  {
    ...
    subtitle: "Conversation to conversion",
    description:
      "The Floo Network is the one magical transport where you must say the destination clearly or you arrive somewhere random — which is the entire thesis of this school. The CRM holds 3,156 school leads and has converted none of them, so every number here starts at zero on purpose: 0 funnel sessions, 0 gates instrumented, 0 active paying schools against a Q3 2026 target of 1. Since 2026-08-22 the lane is instrumented: the gates census measured 377 reachable unmessaged schools (222 verified-mobile, 155 email), the chatbot captures typed identifiers (rate-limited, saasMarketing mode only), the outreach workflow is ACTIVE with its cards in private #hogwarts-funnel, and the roll scripts (hogwarts scripts/funnel/) drain a ramped queue. Conversions are still zero. So the work is not building machinery — it is sending, qualifying on the seven questions a sale requires, giving value before asking for money, and never letting a lead go quiet without a smart next touch. The funnel logic is deterministic and costs zero tokens per lead; the LLM only ever drafts follow-up copy in batches on the Max pool. Code lives in hogwarts/ and mkan/; kun holds the routing and none of the funnel.",
    quote:
      '"follow up" — and the lead that went quiet three weeks ago gets a guide it has never seen, not a "just checking in". Every touch carries something new, or it does not fire at all.',
    spells: [
      {
        name: "funnel",
        effect:
          "The umbrella — with no argument it reports where every lead is stuck and names the biggest stall",
        order: [f("funnel"), s("/funnel"), m("project_lead_engine")],
        steps: [
          "No argument: count every gate and name the gate holding the most stalled leads",
          "The ladder is the UNION of existing enums — never invent a stage name",
          "COLD belongs to /scrape; this lane starts at REPLIED",
          "hogwarts Postgres is the source of truth; Twenty is a mirrored Kanban",
        ],
        connects: ["qualify", "segment", "nurture", "follow up"],
        depends: [],
      },
      {
        name: "qualify",
        effect:
          "The seven facts a sale actually requires, asked one turn at a time — nothing else is asked",
        order: [f("funnel"), s("/funnel")],
        steps: [
          "Authority, student count, country, curriculum, current system, term start, a reachable channel",
          "One question per gate; a gate missing its question, field, asset or timer is broken",
          "You cannot issue an invoice without all seven — that is why each one is asked",
          "The LLM never advances a gate; only a deterministic parse does",
        ],
        connects: ["funnel", "segment"],
        depends: [],
      },
      {
        name: "segment",
        effect:
          "The customer department — a pure function of collected facts that routes owner, cadence, asset and price",
        order: [f("funnel"), s("/funnel")],
        steps: [
          "hogwarts: <authority>-<band>-<rail>-<term>. mkan: <units>-<airbnb>-<wave>-<photos>",
          "Bands come from pricing/config.ts: 100 = free ceiling, 20 = $30 floor, 1000 = enterprise",
          "Rail sets the channel — sd is WhatsApp-first, gulf/eg is EMAIL-first (119 emails vs 45 mobiles)",
          "Never a stored gate: recompute on read so it cannot go stale against its own inputs",
        ],
        connects: ["funnel", "qualify", "nurture"],
        depends: [],
      },
      {
        name: "nurture",
        effect:
          "Give before asking — the value ladder, and the gate/segment pairs that have no gift attached",
        order: [f("funnel"), s("/funnel"), s("/carousel")],
        steps: [
          "Every touch carries a NEW asset; no unsent asset means the touch does not fire",
          "Assets are made by /carousel, /draft and /higgs and referenced BY URL, never generated at runtime",
          "The strongest gives are already built: hogwarts’ sandbox in the school’s own name, mkan’s pre-built listing plus a claim token",
          "Report every pair with no asset — that is a question asked with no gift",
        ],
        connects: ["funnel", "segment", "follow up"],
        depends: [],
      },
      {
        name: "follow up",
        effect:
          "The stall sweep and the smart next touch — the step that decides whether any of the rest worked",
        order: [
          f("funnel"),
          s("/funnel"),
          h("funnel-guard"),
        ...
        steps: [
          "The clock is launchd + Vercel cron, never a Twenty CRON — Twenty is down whenever the Mac sleeps",
          "Touches 1-2 are templated and send unattended; touches 3+ are drafted and need a recorded human yes",
          "Drafting is ONE claude -p session per tick on the Max pool — never one call per lead",
          "Stop on reply instantly: freeze the cadence, advance the gate if it parses, hand to a human",
          "Four touches then DORMANT; at +90d re-enter at the gate it stalled at with an unseen asset",
        ],
        connects: ["funnel", "stalled", "nurture"],
        depends: [],
      },
      {
        name: "stalled",
        effect:
          "Who went quiet, at which gate, for how long — read-only, and the DORMANT sweep",
        order: [f("funnel"), s("/funnel")],
        steps: [
          "Report per gate: how many are past the stall timer and the median days silent",
          "DORMANT is Lead.status=ARCHIVED plus nextFollowUpAt +90d and a dormant:<gate> tag",
          "Nothing is ever deleted — a deleted row is an outreach history nobody can audit",
          "This mode never sends; sending is follow up, and it is opt-in",
        ],
        connects: ["funnel", "follow up"],
        depends: [],
      },
      {
        name: "chatbot",
        effect:
          "The capture surface — the site widget and WhatsApp writing to ONE record through one server action",
        order: [f("funnel"), s("/funnel")],
        steps: [
          "hogwarts already has it and it persists nothing; the work is capture, not construction",
          "Join key is a session cookie until an identifier lands, then inbound:<email> or inbound:wa:<e164>",
          "Capture runs PARALLEL to the LLM call — a capture failure must never block a reply",
          "Check EN/AR dictionary parity and that Arabic-Indic digits normalize, or every Sudanese number is dropped",
        ],
        connects: ["funnel", "qualify"],
        depends: [],
      },
      {
        name: "قمع",
        effect: "Arabic for `funnel` — same lane, which is how Abdout says it",
        order: [f("funnel"), s("/funnel")],
        steps: ["Identical to `funnel` — see its steps"],
        connects: ["funnel"],
        depends: [],
      },
      {
        name: "تأهيل",
        effect: "Arabic for `qualify`",
        order: [f("funnel"), s("/funnel")],
        steps: ["Identical to `qualify` — see its steps"],
        connects: ["qualify"],
        depends: [],
      },
      {
        name: "متابعة",
        effect: "Arabic for `follow up`",
        order: [f("funnel"), s("/funnel")],
        steps: ["Identical to `follow up` — see its steps"],
        connects: ["follow up"],
        depends: [],
      },
    ],
...
export const workflows: Workflow[] = [
  ...
  },
  {
    ...
    description:
      "Measure the gap, then work the highest-yield lane — discovery last, never first",
    steps: [
      {
        keyword: "contact gap",
        action: "Measure — 175 contactable, 2,941 map-pin-only",
      },
      {
        keyword: "leads",
        action: "The 130 contactable tier-A/B schools, unworked",
      },
      { keyword: "enrich", action: "FB About + website — ceiling +40 rows" },
      {
        keyword: "outreach",
        action: "Email first, WhatsApp for the Sudan slice",
      },
      {
        keyword: "pipeline health",
        action: "Stages, tiers, overdue next-actions",
      },
    ],
  },
  {
    ...
    description:
      "A replied lead to a paying customer — count the gates, segment on what was collected, prove every gate has a gift, sweep the stalls, then drain the queue only if someone asked out loud.",
    steps: [
      {
        keyword: "funnel",
        action:
          "Count every gate and name the one holding the most stalled leads",
      },
      {
        keyword: "qualify",
        action:
          "Check the seven facts a sale requires are the questions actually asked",
      },
      {
        keyword: "segment",
        action:
          "Recompute the key; it routes owner, cadence, asset and price path",
      },
      {
        keyword: "nurture",
        action:
          "Find gate/segment pairs with no asset — a question asked with no gift",
      },
      {
        keyword: "stalled",
        action: "Who went quiet, where, for how long; run the DORMANT sweep",
      },
      {
        keyword: "follow up",
