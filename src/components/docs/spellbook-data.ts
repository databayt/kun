...
export const schools: School[] = [
  ...
  },
  {
    ...
    spells: [
      ...
      },
      {
        ...
        order: [f("quality"), p("Browser"), p("Vercel"), p("GitHub"), s("/release")],
        steps: [
  ...
  },
  {
    ...
    spells: [
      ...
      { name: "handover", effect: "UI verification — runs the niche keywords on a URL or every route in a block", order: [f("quality"), p("Browser"), s("/handover")], steps: ["Detect mode: argument starts with / → URL mode (12 niche keywords)", "Bare word → block mode (debug, flow, responsive, lang per route)", "Delegate each pass to the niche keyword definition", "Collect verdicts, write sentinel on PASS", "Report: BLOCKED or READY FOR DEMO"], connects: ["e2e", "check", "test", "release"], depends: [] },
  ...
  },
  {
    ...
    spells: [
      ...
      { name: "type-check", effect: "TypeScript strict checking — Veritaserum for types", order: [f("typescript")], steps: ["pnpm tsc --noEmit", "Report errors with locations", "No JS emitted, pure validation"], connects: ["build", "fix"], depends: [] },
      {
        ...
        order: [f("report"), f("quality"), f("sse"), f("build"), p("GitHub"), p("Browser")],
        steps: [
