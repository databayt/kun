...
const EMPTY: RepoStatus = { openIssues: 0, openDispatches: 0 };

export async function fetchRepoStatus(repo: string): Promise<RepoStatus> {
  // Gated even though this only reads: it reads through the org's PAT, which
  // can see private repos. Being unpublished today is an accident of imports,
  // not a control.
  if (!(await requireContributor())) return EMPTY;

  // Same allowlist as dispatch — `repo` is interpolated into the API path.
  if (!repos.some((r) => r.github === repo)) return EMPTY;
  ...
  const [issuesRes, eventsRes] = await Promise.all([
    ...
    fetch(`https://api.github.com/repos/${repo}/events?per_page=5`, {
      headers,
    }),
  ]);
  ...
  if (issuesRes.ok) {
    ...
    openDispatches = realIssues.filter((i: { labels: { name: string }[] }) =>
