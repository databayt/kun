...
Numbers and hard rules: `.Codex/agents/funnel.md`. Acquisition — everything before the first
reply — belongs to `.Codex/agents/lead.md` and `/scrape`. **kun holds no funnel code.**
...
Product from `.Codex/memory/repositories.json`. Default **hogwarts**. **`mkan` is a different
...
drain       → Codex -p on the Max pool, ONE session per tick, answers written
...
- Whatever was dropped is in `.Codex/logs/funnel-runs.log`.
