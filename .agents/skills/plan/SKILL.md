...
when_to_use: "Use when turning a human-approved spec into an implementation plan — pattern choices, architecture decisions, mirror target, tradeoffs, and risks appended to the feature's GitHub issue — the pipeline Plan stage between /spec and /schema (NOT Codex plan mode). Triggers on: plan the architecture, architecture plan for the approved spec, pipeline Plan stage."
...
Identify the closest existing feature to mirror (the spec usually names one). Read its `content.tsx` / `actions.ts` / `form.tsx` to lock the conventions this feature will follow. Note the canonical pattern cards that apply (`.Codex/patterns/cards/`) and the domain rules (`.Codex/rules/<domain>/`).

### 3. DESIGN — Generate the plan

Delegate the architecture judgment to `tech-lead` (and the user-level `architecture` agent). Produce, using `.Codex/templates/plan.md` as the shape:
