...
The intended outcome of v4 is a kun that knows what it actually contains, wakes itself weekly to allocate work across 4 humans and 14 repos, escalates only the decisions that genuinely require Abdout, stays inside the $200/mo Anthropic envelope on autopilot, and is installable as a single plugin so Ali / Samia / Sedon get the right subset by role.
...
Active. Issue `databayt/hogwarts#115` is the live tracker (admission QA, ~100 checkboxes). Ahmed Baha meeting was 2026-04-04. Today is 2026-04-25; outcome ingestion gap closed by E18.
...
- [ ] Each entry: file path, line count, declared role, primary humans, model selection
**Points**: 2 | **Priority**: P0
...
- [ ] Logs decisions made, deferrals to Abdout, runway delta, pilot stage, what changed
...
- [ ] Runs `qa` on `kingfahad.databayt.org/admission`
...
**Goal**: Hogwarts King Fahad pilot has a structured tracker. Revenue and pipeline don't live in Apple Notes alone.
...
- [ ] `kun-accessible` plugin (E25) tuned for Samia/Ali
...
**Goal**: Move scheduled work from local `scheduled_tasks.lock` to Anthropic-managed Routines. Run when Abdout's MacBook is closed.
...
- [ ] Author `/Users/abdout/kun/AGENTS.md` per Vercel's 20-section structure
