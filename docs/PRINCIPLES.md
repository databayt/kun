...
When a principle conflicts with another, the lower-numbered one wins (priority order is fixed).
...
> Databayt founding principle (from `feedback_kun_autonomy.md`)

We exist to make excellent school operations accessible to every Arabic-speaking community. If revenue urgency ever pushes us to compromise the mission, we cut the revenue path, not the mission.
...
Sprint priorities, feature scope, daily ops → Type 2. Decide fast. Reverse if wrong.
...
## Tier 6 — Databayt-specific

### 21. Open by default

Public repos. Public docs. Public roadmap. Public failures. SSPL is a posture. When in doubt: publish.

### 22. Community is the moat
...
Use `/principle add` skill to log new ones. Use `/principle review` quarterly to challenge the list.
