...
It starts when Twenty CRM gets a new photo. It ends when that photo is on
the website. You only do three of the six steps — the rest happens on its own.

```mermaid
graph TD
    A["1 · Twenty CRM gets a new photo<br/>you drag it onto the home"]
    B["2 · The system picks it up — on its own<br/>saves a copy · opens a job · posts in Slack"]
    C["3 · You make the better photo<br/>drop it in Gemini, save it to your inbox folder"]
    D["4 · The system takes it back — on its own<br/>finds the job it belongs to · resizes it"]
    E{"5 · You look at it<br/>same room? still true?"}
    F["6 · It goes live on the website<br/>and Twenty is updated"]
    A --> B --> C --> D --> E
    E -->|"good"| F
    E -->|"bad — say why"| C
```

You do three things: **put the photo in, make it, say yes.** When you say no,
say why — that is how the next photo comes out better.
