...
```mermaid
graph TD
    S1["A host adds a home on mkan.sd"]
    S2["You write a home into Slack"]
    S3["We add it ourselves —<br/>scraped from Airbnb, or typed in by hand"]
...
| Step | In plain words |
| --- | --- |
| **A home arrives** | Three ways in, and today they are very unequal. **Scraped** — 121 of the 148 homes on the board were pulled from Airbnb; that is how most inventory got here. **By hand** — 26, the real owners we know personally. **From the website** — a host signs up on mkan.sd and adds their own home: it appears on the board within the hour, host and photos too, with nobody copying anything. That last lane is new and has caught nobody yet, because it only just started running. Typing a home into Slack `#mkan` is newer still and being built. |
