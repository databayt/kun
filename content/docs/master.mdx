...
```mermaid
graph TD
    A1["ADD a photo the website does not have yet<br/>— the host just sent it to you"]
    A2["REPLACE a photo already on the website<br/>— it is live, and it looks bad"]
...
| Step | In plain words |
| --- | --- |
| **Add** | The host sends you a photo the website doesn't have. Drag it onto their home in Twenty — nothing else to press. The listing goes from 9 photos to 10, and the new one is mastered on the way in. |
| **Replace** | A photo is already live and looks bad. Fix it where it stands: `pnpm master:queue --listing=1180 --photos=2 --apply`. The better version takes over that same slot, and the listing still has 9 photos. |
| **Never mix them up** | Dropping a photo the website *already shows* into Twenty **adds** it. You would end up with the bad original and its replacement side by side on the page. Add through the CRM, replace with `master:queue`. |
...
| **The job appears in Slack** | `master:dispatch` posts it in `#mkan` with the photo and the words. Until that happens, nobody has been asked — and a daily check now says so. |
...
> **Every step is live.** The CRM step was the last to close (2026-08-24):
> Twenty tells this Mac the moment a photo is added, and the download starts on
> its own. What is *not* proven yet is how many photos survive the gate — see
> the measured trace below.
