...
```mermaid
graph TD
    A1["A new photo you drop in the CRM"]
    A2["A photo the listing already shows"]
...
| Step | In plain words |
| --- | --- |
| **Two ways in** | A photo the listing doesn't have yet goes in through the CRM — drop it on the home and nothing else needs pressing. A photo the listing *already shows* is queued in place with `master:queue`. Never send that one through the CRM: it adds photos, so you would end up with the bad one still sitting next to its replacement. |
