...
```mermaid
graph TD
    A1["Twenty Home<br/>photoStage POOR_QUALITY"] --> Q
    A2["CLI --listing"] --> Q
    Q["master:queue<br/>MasteringRun QUEUED<br/>prompt v1 frozen on row"] --> D
    D["master:dispatch<br/>Slack #mkan task<br/>ASSIGNED"] --> P
    P["master:prep<br/>prompt to clipboard · original to Finder · Gemini opens"] --> H
    H{{"HUMAN — Nano Banana in Gemini app<br/>GATE: billing /decide<br/>Phase 3 swaps only this node"}}
    H --> R{"return lane"}
    R -->|"reply in thread — GATE: Slack scopes"| S1["master:done --from-slack"]
    R -->|"drop in ~/mkan/inbox"| S2["master:relay"]
    R -->|"newest ~/Downloads"| S3["master:done runId"]
    S1 --> V
    S2 --> V
    S3 --> V
    V{"validate<br/>not byte-identical?<br/>size/ratio ok?"} -->|"tech fail"| F["FAILED + Slack alert"]
    V -->|"pass"| ID{"identity<br/>does render depict THIS room?"}
    ID -->|"no"| RJ["master:reject<br/>REJECTED, attempt+1"]
    ID -->|"yes"| M["MASTERED<br/>WebP to CDN"]
    M --> U["swap photoUrls by URL match<br/>UPDATED · Twenty rollup · Slack ✅"]
    RJ --> Q
    F -.->|"requeue-failed"| Q
    U -.->|"host edits photos"| DR["DRIFTED (reconcile/status)"]
    RC["master:reconcile<br/>launchd 10:00 · stall clock"] -.->|"re-dispatch stalls"| D
```

The ops set outside the happy path: `master:reject` / `master:revert` / `master:status` / `master:reconcile` / `master:census`.
