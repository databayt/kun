...
```
Twenty Home (photoStage Kanban) ─┐        CLI --listing ─┐
                                 ▼                       ▼
master:queue     → MasteringRun rows (mkan Prisma, prompt FROZEN v1 per row)
master:dispatch  → Slack #mkan task: original unfurled + copyable prompt + return command
master:prep      → prompt→clipboard · original→Finder · Gemini opens   (~20s human floor)
HUMAN            → Nano Banana in the Gemini app (covered by Google AI Pro, $0 API)
master:done      → validate → sharp ≤2048px 4:3 WebP → CDN → swap by URL match → UPDATED
                 → Twenty rollup (photosMastered, cover, photoStage) → Slack thread ✅
master:reject / revert / status / reconcile / census — the ops set
```
