...
A home arrives. A bad photo goes in. A better photo comes out on the website.
You touch one step.
...
graph TD
    ...
    ADD["A photo the website doesn't have:<br/>drop it on the home"]
    REP["A photo already there that looks bad:<br/>mark the home 'poor quality'"]
    JOB["The photo gets its own job,<br/>and shows up in Slack"]
    YOU["You paste it into ChatGPT,<br/>look at the result, and save it"]
    Q{"Same place, and the right room?"}
    NO["Say what was wrong.<br/>It tries again."]
    YES["It goes live.<br/>The old photo is kept, always."]
...
Every home ends up in the CRM, however it arrived, and that is where a photo is
sent for mastering. Saving the new photo **is** the yes — if it came back wrong,
don't save it, press enter again.
...
| **The CRM** | Every home in one place, and the only place mastering starts. You never open a terminal to begin. |
| **A new photo** | The host sends you a photo the website doesn't have. Drag it onto their home. The listing goes from 9 photos to 10, and the new one is mastered on the way in. |
| **A bad photo** | It's already live and it looks bad. Set the home's stage to **poor quality**: its photos are queued and one is handed to you in Slack. |
| **Don't mix them up** | Dropping a photo the website *already shows* onto the home **adds** it — you'd end up with the bad original and its replacement side by side. New photo: drop it. Bad photo: flag it. |
| **Its own job** | One photo, one try, one row. The exact words sent to ChatGPT are saved on that row and can never change afterwards, so you can always see what a photo was asked for. Only one photo per home is handed out at a time. |
...
| **It goes live** | The new photo takes the old one's place and gets a proper name: `mkan/0001-01/bedroom-1.webp`. The original is never deleted — it keeps its old address forever. |
| **It tries again** | Saying no needs a reason. That reason is the only thing that makes the next round better. |
| **Nothing goes quiet** | Every morning at 10:00 a check tells Slack about anything stuck: a job nobody picked up, a job nobody was ever asked to do, a photo that quietly fell off a listing, a saved photo nobody read. |

> **Every step is live.** The two newest: a home added on mkan.sd now reaches
> the board on its own, and marking a home poor quality now starts the loop for
> the photos it already shows. What is *not* proven yet is how many photos
> survive the check — see the measured trace below.
