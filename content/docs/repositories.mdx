...
**`bootstrap` does this for you** — step 14 (`sync-repos.{ps1,sh}`) clones all 11 repos to `~/<name>`. To re-sync at any time: `& "$env:USERPROFILE\.claude\scripts\sync-repos.ps1"` (Windows) or `~/.claude/scripts/sync-repos.sh` (macOS/Linux).
