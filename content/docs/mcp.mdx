...
**Every machine gets the full `mcp.json`.** Config is universal — any machine can run any task. Access is scoped by **secrets, not config**: a server whose API key isn't in your `~/.claude/.env` simply doesn't connect, and which keys you hold is set by the **Gist** you're handed. Canonical installer: `setup.sh`.
