...
|--------|---------|---------|
| `ANTHROPIC_API_KEY` | Claude Code + Agent SDK | Core engine |
| `GITHUB_TOKEN` | GitHub MCP, repos, PRs | github MCP |
| `VERCEL_TOKEN` | Deployment automation | vercel MCP |
| `SLACK_BOT_TOKEN` | Slack messaging | slack MCP |
| `SLACK_WORKSPACE_ID` | Slack workspace target | slack MCP |
