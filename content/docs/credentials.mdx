...
```bash
credentials.sh set GITHUB_PERSONAL_ACCESS_TOKEN ghp_xxxxxxxxxxxxx
```
...
```bash
credentials.sh get GITHUB_PERSONAL_ACCESS_TOKEN
```
...
```bash
# Store
security add-generic-password -a "kun" -s "GITHUB_PERSONAL_ACCESS_TOKEN" -w "ghp_xxx" -U

# Retrieve
security find-generic-password -a "kun" -s "GITHUB_PERSONAL_ACCESS_TOKEN" -w
```
