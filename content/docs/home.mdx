...
The site account is provisioned when the first home goes `live`: `username` is the host's own
name (it is what a guest reads under *"Hosted by"*), the password is the shared one, and the
number is what they log in with — `getUserByIdentifier` resolves a bare `0006`, so no host is
ever handed an email address. `mkanUsername`, `mkan account` and `Account provisioned` are
filled on the host record at that moment.
