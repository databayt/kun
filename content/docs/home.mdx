...
| The first real home | **awaiting the first message** — the reader was proven on the example (8.7 s, every field right); the channel run is proven on non-home chatter only |

**Blocked on nothing.** The one human action this page opened with — the Slack read scopes
on the `kun` app — was clicked the same day: `groups:history` + `files:read` added, the app
reinstalled, and the probe that used to answer `missing_scope` now reads the channel. That
click also unblocked the mastering phone lane (`master:done --from-slack`). What remains is
proof: the first real home typed into `#mkan`.
...
The first real message, and the first `live`. Then the one-line hardening of the site's
webhook (mint the code on `LIVE`). Phase 2 — the bell from Twenty for edits made in the CRM's
