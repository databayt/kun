...
**How the account is assigned, in order.** The phone in your words is the anchor: it is
normalised (`٠٩١٢…`, `00 249 …`, `+249 …` all become one number) and looked up on the `host`
records. A known host keeps the account written on their own record (`mkanUsername`, `0004` for
السند), falling back to what their homes carry; a host nobody knows gets a new `host` record and
the next free manual number (`0005` today; `1001+` stays for scraped hosts). No phone means
nothing is minted — the thread asks for the number first. The site's `NNNN@mkan.org` account is
created only when the first home goes `live`.
