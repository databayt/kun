...
| --- | --- | --- |
| a phone number, in any digits | `hostPhone` as `+249…` — and the host record it belongs to | **yes** — someone to call |
| a person's name | `hostName` | expected, not required |
| `غرفتين` · "2 bedrooms" | `bedrooms` | **yes** |
| `حمام` · `حمامين` · "3 bathrooms" | `bathrooms` | **yes** |
| `شقة` · `فيلا` · `استوديو` · "villa" | `propertyType` (`APARTMENT`, `VILLA`, `ROOMS`…) | **yes** |
| `٣٠ الف الليلة` · "30k/night" | `priceNightSdg` | **yes** |
| a neighbourhood · a map link | `zone` (one of the 45 Port Sudan zones) · `googleMapsUrl` | **yes** — one of the two |
| a title, a description | `titleAr`, `descriptionAr` (English mirrored later) | **yes** — drafted for you from the facts if you did not write them; you see the draft before `live` |
| `صالة` · `مطبخ` · `مكيف` · "wifi" | `highlights` · `amenities` | nice |
| "sleeps 4" · `٤ اشخاص` · `سريرين` | `guestCapacity` · `beds` | nice |
| `1.` `2.` `3.` — numbered lines | one home per number, same host, codes in order | identity |
| "price confirmed" · `السعر مؤكد` — you agreed it with the host in person | `priceConfirmedByHost` | part of the bell |
