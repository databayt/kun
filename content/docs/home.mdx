...
### Which job the words are

The channel is one desk with several jobs, and which job a message is gets decided in plain
code before the reader is asked anything — because a correction read as fresh notes becomes a
second copy of a flat that already exists.

| You write | It is | What happens |
| --- | --- | --- |
| a name, a phone, rooms, a price | **new** | a host record, an account, a code per unit |
| **`0004-02`** and some words | **a change to that home** | only what your words change; the rest is left alone |
| **`0004-02`** and nothing else (or just its link) | **a question** | the home's card comes back — what it holds, what is missing, whether it is live. No reader, no cost |
| `0004-03 و 0004-02 السعر ٦٥` | **a change to both** | codes match your words in the order you wrote them |
| **`live 0004-02`** · `انشر` | **publish** | wins over the code it carries |
| `same 0004-02` · `new` | **a verdict** | answers the "is this the same flat?" question, in its thread |
| a reply inside a home's thread | **a change to that home** | no code needed — the thread already knows |
| a fix with no code (*"الحمامات بقت تلاتة"*) | **a change to *which*?** | the desk asks for the code rather than guessing |
| anything else | **chatter** | silence |

A code only counts when the CRM actually has it. That is what keeps `2026-08` in a date, or a
typo, from taking over a message — an unknown code is not a code, and the words are read as
notes instead.
...
6. The same thread then carries the **account handover** — the WhatsApp the scout sends the
   host, ready to tap: the account number, the shared password, the login link and the
   listing link(s), behind a `wa.me` link that opens the host's chat with the message already
   typed. The machine sends nothing; the scout taps, sends, and says so with
   `pnpm crm:handover --account=0005 --mark-sent --apply` (`credentialsSentAt` on the host,
   plus a Note carrying the message). `pnpm crm:handover --account=0005` re-prints it any
   time, `--all` is the sheet for every site-provisioned host, and a number that does not
   read as a Sudanese mobile is reported rather than dialled. Until 2026-08-29 this step did
   not exist: the docs said the host is told the number and the password, and nothing ever
   told them.
...
`0006` and the shared password are the whole of what a host is told — and since 2026-08-29 they
are actually told it, in the `live` thread (step 6 of [Saying live](#saying-live)). `User.email` is a required
...
| The handover | **built 2026-08-29** — `scripts/crm/host-handover.ts`: the shared-password message (number · password · login · listing links) compiled per account from the site (truth) and Twenty (contact), posted in the `live` thread by `home:publish`, re-printed by `pnpm crm:handover`; `--mark-sent --apply` stamps `credentialsSentAt` + a Note. Dry-run sheet on the four hand-verified hosts: 3 ready, 1 (`0003`) whose recorded number is not a mobile. No host has been handed an account yet |

**Blocked on one small click, and only for the Hermes ear.** The read scopes were added the
same day and the timer ear has carried every home end to end since. Hermes itself has never
woken: `grep -c C0BS2NZE2AY ~/.hermes/logs/agent.log` is still `0`, because the app is not
subscribed to the event that fires for a private channel. api.slack.com/apps → the `kun` app
→ *Event Subscriptions* → Subscribe to bot events → `message.groups` (and `message.mpim`,
which the gateway warns about on every start) → Save → Reinstall. The steps, and why not to
paste a whole manifest over the app, are in `mkan/scripts/crm/hermes/SLACK-EVENTS.md`. Until
then the two-minute timer is the listener, which it has proven it can be — the cost is
...
| mkan `36ddf23` · hogwarts `5de61e94d` | **The account handover** — `scripts/crm/host-handover.ts` + `public-links.ts`: the shared-password message per site account, posted in the `live` thread by `home:publish`, `--mark-sent --apply` stamps `credentialsSentAt`; `claim-tokens.ts` stopped printing localhost links, `--account=` selects by the number on the sheet, the gift sheet flags a link dying within 48h. Same day the hogwarts opening-message chain was re-proven end to end (see [Funnel](/docs/funnel)) |
