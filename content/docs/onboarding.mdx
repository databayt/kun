...
- **All 11 databayt org repos at user-home root** — `~/codebase`, `~/kun`, `~/hogwarts`, `~/souq`, `~/mkan`, `~/shifa`, `~/shadcn`, `~/radix`, `~/swift-app`, `~/distributed-computer`, `~/marketing`. No `oss/` subdirectory; same on Windows, macOS, Linux. See [repositories](/docs/repositories#local-layout).
