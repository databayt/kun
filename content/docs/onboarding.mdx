...
| **Hermes** | `h` | Optional **assistant gateway** (Slack/WhatsApp/Telegram channels) — NOT a coding CLI. NousResearch's [hermes-agent](https://github.com/nousresearch/hermes-agent) | n/a — its interactive setup (`hermes gateway setup`) runs later, never headless. See [Hermes](/docs/hermes) |
...
| **Hermes** *(optional)* | `hermes` CLI (official installer) + `h` launcher; gateway setup stays manual (interactive) — see [Hermes](/docs/hermes) |
...
| 5 | Agents — the selected lanes from the multi-select: Claude Code CLI + Desktop (Mac/Win) + MCP symlink, Antigravity (`~/.gemini/` bridged to the same fleet), opencode (+ permissive config), Hermes (CLI only); one managed launcher block `c`/`a`/`o`/`h` |
...
**The other lanes.** Beside Claude Code sit three more agents (see [The agents](#the-agents)): **Antigravity** (`a`) — Google's terminal agent wired to the same MCP fleet/skills/playbook via `~/.gemini/`, the fallback + cheap lane (full detail: [Antigravity](/docs/antigravity)); **opencode** (`o`) — open-source terminal agent, tertiary; and **Hermes** (`h`) — the optional chat-app gateway, Slack-wired (full detail: [Hermes](/docs/hermes)). Keep architecture and complex work on `c`.
...
2. **Refresh** `~/.claude` — `setup.sh --quiet` re-copies agents/skills/rules/settings/MCP from the fresh source and prunes engine items removed upstream
3. **Health-check** — full `health.sh` audit + a disk-space floor
4. **Record** — state to `~/.claude/.kun-maintain.json` (`cat` it anytime)
5. **Report** — posts to the team's [Config Health Dashboard](https://github.com/databayt/kun/issues?q=label%3Aconfig-health) weekly, and immediately if health goes RED
...
| `hermes` installed but no channels | Hermes gateway setup is interactive by design: `hermes gateway setup && hermes gateway start` — Slack wiring: [Hermes](/docs/hermes) |
