...
| Windows: `'iwr' is not recognized as an internal or external command` | You pasted into `cmd.exe`. Use the universal form: `powershell -ExecutionPolicy Bypass -Command "iwr https://kun.databayt.org/install.ps1 | iex"`. Or open **PowerShell** (Start → "PowerShell") and re-run the original line. |
