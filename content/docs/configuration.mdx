...
| Setting | Value |
|---------|-------|
| Model | Opus 5.5 (`claude-opus-5-5`, the `opus` alias) — canonical in `.claude/engine.json` |
| Fallbacks | Opus 5 → Sonnet 5 (never Fable — headless Fable can bill usage credits) |
...
Three axes: **model tier × effort × product lane**. The main loop runs Opus 5.5 (Fable 5.1 is a `/model` escalation, never a default); agents resolve a tier alias.
