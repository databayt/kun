...
| Model | Use Case |
|-------|----------|
| **Opus 4.8** (1M context) | Architecture, complex features, review |
| **Sonnet 5** (1M context, Claude Code default) | Fast iteration, routine changes |
| **Haiku 4.5** | Search, lookups, exploration |
| **Fable 5** | Top-tier reasoning (Mythos class) |

Kun defaults to **Opus 4.8** for all agents (canonical in `.claude/engine.json`). Haiku for explore subagents only. API prices live at [platform.claude.com pricing](https://platform.claude.com/docs/en/about-claude/pricing) — we don't hardcode them here; billing is subscription-only anyway.
