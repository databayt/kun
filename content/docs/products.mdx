...
| **Subagents** | 47 specialized agents |
...
Kun runs **Fable 5** in the main loop (canonical in `.claude/engine.json`), with Opus 4.8 → Sonnet 5 as fallbacks. Agents carry a **tier alias** (`opus` / `sonnet` / `haiku`), never a pinned version, so the fleet upgrades in one line. API prices live at [platform.claude.com pricing](https://platform.claude.com/docs/en/about-claude/pricing) — we don't hardcode them; billing is subscription-only anyway.
