...
| Property | Value |
|----------|-------|
| Workspace | `databayt-sh` |
| Invite Link | [Join Slack](https://join.slack.com/t/databayt-sh/shared_invite/zt-42mt16sb7-urQm0MDGtPMEsdrd0Uf3fw) |
| Team | Abdout, Ibrahim, Moutaz, Aseel, Moed, Samia, Sedon |

Share the invite link with team members to join. Use your databayt identity (`<name>@databayt.org` — same names as the [CRM](/docs/crm) logins).

> Previous workspace `hogwarts-wve9301` is retired — everything team-wide lives in `databayt-sh` now.
...
| `#social` | Social drafts, approval links, publish and drain notices. Set as `SLACK_HOME_CHANNEL`. **Keep it private** — its messages carry one-click publish links whose only protection is unguessability |

**Slack is the one *communication* channel in the [social](/docs/social) taxonomy** — it is where
drafts get approved and posts get announced, and it is never itself a publish destination. That is
a deliberate line: the eight distribution channels reach an audience, Slack reaches the team.
