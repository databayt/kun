...
Layer 1: Foundation (Anthropic-provided)
  Opus 5.5 | Fable 5.1 | Sonnet 5 | Haiku 4.5 | 1M Context | Subscription
```
...
### Agent Fleet (53 agents, two plugins)

```
captain (company brain — allocates, decides, never writes code)
├── Business:   revenue, growth, support, analyst, lead, funnel
├── Delivery:   product, tech-lead, ops, guardian, cloudflare, pwa
├── Engine:     quality, record, report, analyze, learn, package, clone
├── Content:    transcribe, adjudicate
...
| Fleet | Plugin | Agents |
|-------|--------|--------|
| **Company** (26) | `kun-company` | captain, revenue, growth, support, analyst, lead, funnel, product, tech-lead, ops, guardian, cloudflare, pwa, quality, record, report, analyze, learn, package, clone, transcribe, adjudicate, hogwarts, souq, mkan, shifa |
...
### Rules Engine (38 domain rules + 5 cross-cutting)

Domain rules auto-load via a `paths` glob: `react-perf` (8), `next-16` (6), `react-19` (5), `prisma-6` (4), `tailwind-v4` (4), `typescript-strict` (4), `authjs` (3), `neon` (2), `s3` (2). Five more load unconditionally: `github-workflow`, `block-protocol`, `patterns`, `cowork-bridge`, `engine-parity`.
...
| **Abobaker** | CEO | Executive strategy, company operations, capital allocation |
