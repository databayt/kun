...
- **The Google lane is blocked on billing, not on a key** (probed 2026-08-19). The key exists,
  is in the central `.env` and in Vercel production, authenticates fine, and lists all six image
  models — `GET /v1beta/models` returns 200. Generation is still refused:

  ```
  ✗ No image/video quota on this key — the free tier for generation is limit: 0.
  ```

  Text models work on the free tier; image and video are hard-zero. The fix is one switch —
  **AI Studio → Set up billing** on the project, with a monthly spend cap set at the same time.
  The probe costs nothing: the gate refuses before generating. It remains new external API spend,
  which the subscription-only posture says is Abdout's call, not a session's.
