...
> **The free tier is unsettled.** Google's pricing page lists *Free tier: Not available* for
> every current image model, while third-party trackers report ~500 images/day at 1K through
> `gemini-2.5-flash-image` on an AI Studio key. We have not tested it with our own key. The
> first run on a new key should be `--model legacy --size 1K`, watching for a 429 versus a
> charge. Until then, assume the lane is paid.
...
**Google lane** — one line in the central `.env`: `GEMINI_API_KEY=` from
[AI Studio](https://aistudio.google.com/apikey). Nothing to install; the script uses `fetch`.
