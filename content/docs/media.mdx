...
### Scenario 1: Mkan Port Sudan Property Tour & Listing Pack {#scenario-mkan-walkthrough}
...
- **Workflow:**
  ...
  2. Select **Walkthrough** (`9:16` · Video · `Seedance 2.5` / `Veo 3.1`) — generates 35mm handheld coastal drift from interior to Red Sea balcony @ 24fps with coastal wind & tea foley.
  3. Select **Post** (`4:5` · Image · `Gemini 3.1` / `GPT Image 2`) — generates 50mm architectural framing with morning sunlight and clay accent (`#e05638`).
...
### Scenario 2: Hogwarts Academic Admission Campaign Pack {#scenario-hogwarts-admission}
...
### Scenario 3: Airbnb Swipe File & Repurposing for Mkan {#scenario-airbnb-repurposing}
...
### Scenario 4: Fast Deterministic Text-Bearing Cards {#scenario-deterministic-canvas}
...
### Scenario 5: High-Impact Video Generation {#scenario-video-production}
...
- **Workflow:**
  ...
  3. Run Google Veo Lite ($0.40 / 8s clip):
     ...
     ```
  4. Upload finished render to [Brief Queue](https://kun.databayt.org/en/social/media#brief-queue).

### Scenario 6: Social Pipeline Hand-off & Cross-Platform Broadcast {#scenario-social-bridge}
