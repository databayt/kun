...
|-------|-----------|------|------|-------------|
| **Generate** | Claude (`/social` skill) · the Hub's **Draft** tab · `/higgs` for media | `lib/social-draft.ts` (cron lane) · `SocialDraftRequest` + `draft` queue mode (Hub lane) | A · B · C | Claude-native. The relays never write copy. The Hub lane drains on a 5-minute launchd tick (`scripts/drain-drafts.sh`) — heartbeat every look, Claude only when asks are pending. |
