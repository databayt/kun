...
- **Opt-in:** `SOCIAL_AUTOPOST_PRODUCTS` is **empty by default** — the cron authenticates, does
  nothing, and says so.
- **Known limits, stated rather than hidden:** the link is **replayable until it expires**
  (clicking twice posts twice) and anyone holding it can publish. Both are consequences of having
  no `SocialPost` table yet. Keep the review channel private.
