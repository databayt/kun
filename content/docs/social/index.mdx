...
calendar ─► draft ─► media ─► approve ─► publish ─────────► measure
    │         │        │         │          │                  │
  which     Claude   /higgs    human    drain | hermes    SocialMetric
brand,when  AR-first text-free  gate    | copy-out         + UTM tags
                              ▲ HARD STOP
```

| Stage | Owner | Notes |
|-------|-------|-------|
| **Idea** | `calendar` skill · `weekly` cadence · `growth` agent pillars | What to say, to whom, when |
| **Draft** | `draft` skill — **Claude, natively** | Arabic-first bilingual copy, one variant per channel, written against [Brand & Voice](/docs/brand) |
...
| **Schedule** | `SocialVariant.scheduledFor` + `/api/social/drain` | The composer takes a date and time; the drain publishes due variants every ~15 minutes from GitHub Actions |
...
Six stages, six words. Each runs on its own; `social` chains all of them.
...
| **`publish`** | Deliver | "publish it" · "schedule it for tomorrow" |
...
The `growth` agent owns the strategy layer above them; `.claude/vocabulary.json` holds all six in
the *Pensieve* school. No slash needed — say the word.
...
- **Removed** — the in-app AI draft card: gateway-LLM drafting violated the layer boundary, so
  drafting now lives in the `draft` skill (Claude-native).
...
- `/higgs` skill — media generation · `calendar` · `draft` · `approve` · `publish` · `measure` · `growth` agent.
