...
From nothing to a live page, in one box. Everything above the dashed line happens on
`/social/publish` without leaving it — which is why the bare `/social` opens there.

```mermaid
flowchart TD
  P["Press a brief from the plan<br/>audience · CTA · constraint written"] --> ASK
  W["…or say what it is about"] --> ASK

  ASK["Ask — brand, feature, shape, channels<br/>and the knobs ride with it"]
  ASK --> C{"Craft gate<br/>reads the writing"}
  C -- "banned word · bare link · too short" --> ASK

  C -- "clean" --> D{"The copy lands in the field.<br/>Right yet?"}
  D -- "no — say what to change" --> RF["Refine — the next turn<br/>of the same draft"]
  RF --> D
  D -- "yes" --> M["Attach media (optional)"]

  M --> S{"Send"}
  S -- "Now" --> F
  S -- "Later" --> Q["Queued for the time you picked"] --> F
  S -- "Review" --> L["One signed link per channel"] --> F

  F["Every wired channel, one press"] --> G["Save the receipt — the post id"]
  G --> H["Read the numbers back, six-hourly"]

  classDef start fill:#1d2436,stroke:#4d5a7a,color:#d7deee;
...
Two ways in, and the first one costs no typing at all: `content/social/pillars.json` holds each
brand's recurring briefs — audience, call to action, constraint, angle and scene already written —
and pressing one asks with it verbatim. The [Calendar](#the-hub-is-this-table-as-routes) stage
renders the same file as the week's plan; the composer offers it as a starting line.

**The loop in the middle is the point.** A draft that comes back eighty percent right used to mean
retyping it by hand — eighty percent right being the common case. Saying what to change files the
next turn of the same thread: the parent is superseded, the child carries the root brief and the
inherited knobs, and only the newest is `answered`, so the queue shows the current draft rather
than three versions competing for a slot.
