...
- **The composer is an editor** (2026-07-30) — the **Publish** tab wears the agent's shape: the same
  ...
  Behaviour unchanged — same three actions, same validation, same per-channel outcome list.
- **Still pending** — adapters beyond Slack (see [Hermes](/docs/hermes)), per-brand Telegram
  channels, a PostHog destination to consume the UTMs, per-channel drafting in the cron, and
  **nothing draining the draft queue on a schedule** — an ask waits for the next session someone
  happens to run, and the window polls without a timeout, so "nobody ran a session today" and "the
  app is broken" look identical from the Hub. Tracked in `docs/SOCIAL-AUTOMATION.md`.
