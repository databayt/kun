...
`/social` is one job split across pages, and **`/social/publish` can do the whole job by
itself** — which is why the bare `/social` opens there. The other three exist for when you
want to start somewhere earlier.

```mermaid
graph TD
    CFG["Settings —<br/>Brand · Feature · Channel · Post · Draft · Media · Queue · Plan"]
    CAL["/social/calendar<br/>this week's topics, ready to press"]
    DRA["/social/draft<br/>ask for words in your own words"]
    MED["/social/media<br/>find a picture, or make one"]
    PUB["/social/publish<br/>read it · attach it · send it"]
    MEA["/social/measure<br/>how it did"]
    LIVE(["Live on your channels"])
...
Nothing is lost moving between them: the copy you are working on, the pictures you have
attached, **and every one of those settings** follow you from page to page. Pick a brand on
Draft and Publish is already publishing as that brand.

### The settings each page shows

One row, one set of answers — but a page only shows the words that mean something where you
are standing. A stage that cannot send has nothing to schedule; a stage that only finds
pictures has no post shape to pick.

| Word | calendar | draft | media | publish | Why |
| --- | :---: | :---: | :---: | :---: | --- |
| **Brand** | ● | ● | ● | ● | Everything is per-brand: whose topics, whose voice, whose library, whose Page. |
| **Feature** | ● | ● | ● | ● | Which part of the product. It narrows the topics, grounds the writing, and filters the library. |
| **Channel** | — | ● | — | ● | Where it lands. It changes what gets written, so Draft needs it; nothing on Media or Calendar turns on it. |
| **Post** | — | ● | — | ● | The shape — words, one image, a carousel, a reel. It changes what the writer is told to write. |
| **Draft** | — | ● | — | ● | On Publish, the drafts already waiting. On Draft, its own three: angle, register, model. Same seat, different question. |
| **Media** | — | ● | ● | ● | Which pictures are in play. On Media it also narrows the results, and the count on the bar says by how much. |
| **Queue** | ○ | — | — | ● | What is already asked for and unanswered. Calendar shows this per brief as a chip, so it belongs — it is not wired into the row yet. |
| **Plan** | — | — | — | ● | Now, later, or out for approval. Only a stage that can send has any use for it. |

● shown · ○ belongs, not wired yet · — deliberately absent

**Draft** and **Media** grew this row on 2026-08-26; before that Draft's seat held three
writing knobs and no brand, and Media's seat did not exist — so a person asking for copy had
to leave the box they were writing in to say which brand it was for. **Calendar** is the one
page still without it: Brand is currently a page-level context rather than a word in the row,
and Queue would earn its place there the moment it is.

| Page | What you go there for |
| --- | --- |
| **calendar** | The topics we already decided to write about this week, per brand, with the ones due now highlighted. Press **Queue now** and it becomes a request for words. |
| **draft** | The place to ask for copy in a sentence and get Arabic and English back. Type again to refine what is on screen rather than starting over. **Use Arabic** carries it to publish. |
| **media** | Type two words and the library narrows; **Attach** puts a picture in the tray. Generating something new lives below it — this page finds what exists, the studio asks for what does not. |
| **publish** | The whole job in one place: write it, refine it, attach, and send — now, later, or out for someone else's approval. It is also the review queue, where a draft someone else asked for waits for a yes. |
| **measure** | The last twenty posts, with what each one reached. |

## How a post travels

The fourteen steps below all happen on **`/social/publish`** — this is that one page,
opened up. Eight of them are yours — 1 to 4, then 8 to 11 — and every one of those is a tap
or a few words.

```mermaid
graph TD
    S1["1 · Open the box"]
    S2["2 · Set the post<br/>brand · feature · channel · shape"]
    S3["3 · Say the topic<br/>(or press one from /social/calendar)"]
    S4["4 · Press Write it"]
    S5["5 · Every pick becomes<br/>a line of the prompt"]
    S6["6 · Claude writes it<br/>in Arabic and English"]
    S7["7 · A craft check<br/>reads it first"]
    S8["8 · You read it"]
    S9["9 · Say what to change"]
    S10["10 · Attach a photo or a video<br/>(the tray, or /social/media)"]
    S11["11 · Say when"]
    S12["12 · It posts to<br/>your channels"]
    S13["13 · It saves the receipt"]
    S14["14 · It reads the numbers back<br/>(shown on /social/measure)"]
...
**2 · Set the post.** One row behind the seat, four answers, and none of them a form field
— each is a paragraph of context handed to the writer.

- **Brand** · Balqalam, Mkan, databayt, Sijillee, Moallimee. (Hogwarts writes through
  Balqalam, its Arabic face, so it is not in the picker.) The biggest single pick — see
  [why](#why-this-beats-writing-it-yourself) below.
- **Feature** · which part of the product the post is about: Admission, Attendance, Exams,
  Grades… That brand's own block registry, narrowed to the blocks a post can be about —
  twelve for Balqalam, six for Mkan, four for databayt, none yet for Sijillee or Moallimee.
  Optional, and it is the difference between a post about a school system and a post about
  attendance in one.
- **Channel** · where it is going. A channel is offered only when both the transport and
  that brand's own destination are wired, so an unpublishable channel is never on the list.
- **Post** · the shape. Words alone · a caption beside one image · one caption over several
  · a video · an image or video with no words · a swipe deck · a story · a reel. Nine of
  them, and each changes what the writer is told to write.

They stay set between visits and follow you across the stages, so this is a step you do
once and then rarely touch — which is why it is one box and not four.

**3 · Say the topic.** A few words is enough — *"attendance"*. Or press one of the week's
...
**4 · Press Write it.** The seat on the right of the bar. That is the last thing you do
until there is a draft to read.

**5 · Every pick becomes a line of the prompt.** This step is the whole point of the
pipeline, and it is spelled out [below](#why-this-beats-writing-it-yourself).

**6 · Claude writes it.** Arabic written first and English mirrored from it — a sibling,
...
**7 · A craft check reads it first.** It refuses on its own — it is not advice. A banned
...
**8 · You read it.** The copy lands in the same field you typed the topic into.

**9 · Say what to change.** Eighty percent right is the normal case, and it used to mean
...
**10 · Attach a photo or a video.** Optional. ⊕ opens the library — pick something that
...
**11 · Say when.** **Now** sends it. **Later** books a time, and a sweep every ~15 minutes
...
**12 · It posts to your channels.** One press fans out to all of them. Today that means
...
**13 · It saves the receipt.** The platform's own post id, so the post can be taken down
again. Every link is tagged so its clicks can be counted.

**14 · It reads the numbers back.** Reach and engagement, every six hours.
...
## The other way a post travels

Everything above is **you driving it** — you open the box, you say the topic, you press
Write it. There is a second path where nobody opens anything: **the week runs on timers,
and you only say yes.**

Seven steps instead of fourteen, and you touch exactly one of them.

```mermaid
graph TD
    W1["1 · Monday 07:00<br/>the week's topics are picked<br/>for all four brands"]
    W2["2 · Every 60 seconds<br/>the agent writes it —<br/>Arabic first, English mirrored"]
    W3["3 · A craft check reads it<br/>before you do"]
    W4["4 · You read it and say yes<br/>post now, or pick a time"]
    W5["5 · Every 15 minutes<br/>it goes out to<br/>the Facebook page"]
    W6["6 · Every 6 hours<br/>it reads the numbers back"]
    W7["7 · Friday 09:00<br/>a digest lands in Slack:<br/>what shipped, what is waiting"]
...
Both paths end in the same place, and they share the same queue — a post seeded on Monday
and a post you typed yourself both wait on `/social/publish` for the same yes.

### What happens when nobody says yes

Step 4 is the only box with no timer on it, which makes it the only place the week can
stop. It already did, for **27 days**.

```mermaid
graph TD
    Q1["A draft is ready<br/>written and checked"]
    Q2["It waits for you<br/>you were told once, on the day"]
    Q3["Nothing is ever scheduled"]
    Q4["The sender looks for work<br/>and finds an empty queue"]
    Q5["It reports success<br/>every light stays green"]
...
Nothing broke. Every job ran and honestly reported success, because each one truthfully
emptied a queue that nothing had reached. **Green meant "nothing to do", not "it worked."**
A daily nudge now says what is waiting, and stays quiet when nothing is.

**[The loop →](/docs/social/loop)** — what each box does, and the one thing to get right
when you approve.
...
|-------|--------|---------------|
| **/social/calendar** | Plan | The same box, searching the plan: type two words and the briefs narrow, and pressing one files it as a draft ask. **This ISO week's rotation picks** sort first (the same math the Monday seeder runs). The full plan stays underneath — every brief in the file's own order, a queue chip on each (in the queue · answered · approved · dismissed) and **Queue now** on the ones nobody has asked for |
| **/social/draft** | Draft | The agent window, and since 2026-08-25 the same box the other stages wear: one glass bar whose field is a BRIEF, its ⊕ the shared media tray, its empty seat **Publish's settings row** — Brand · Feature · Channels · Post · Direction · Media, every one of which changes what gets written. Timing and Queue are absent: this stage cannot send. Typing again refines the draft on screen rather than starting another. The answer, its media and the "Use …" buttons sit under it — "Use English" is the only place the English half is reachable |
| **/social/media** | Media | The same box, finding rather than asking: type two words and the library narrows, and each row attaches into the shared tray. Its seat carries the settings row too — Brand · Feature · Media — which narrow the results, with the bar's count saying by how much. The showroom below it still browses — shelves by brand, cards with model, credits and source. Media Studio (generation) sits under the gallery: the box finds what exists, the studio asks for what does not |
...
| **/social/measure** | Measure | The same box, searching the ledger: type two words — a phrase from the copy, a channel, or a status — and the row is there with its channel, status, time and reach on it. Nothing is pressable, because a variant is a record of a decision already taken. The last 20 variants stay as a table underneath |

Cross-stage state lives in the layout's provider, which persists across stage navigations: the
agent's queue poll runs on for minutes, the editor keeps its copy, **the post's settings —
brand, feature, channels, shape, media filters — are the same on every stage**, and the
**attachment tray** survives a trip to the showroom and back — switching stages must not throw any of it away.
