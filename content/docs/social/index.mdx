...
**The words** arrive one of two ways: a contributor files a brief on `/social`, a Claude
Code session drains the queue and writes the copy back through the craft gate
(`scripts/social-drafts.mjs answer`), or a reviewer edits copy directly in the queue's
editor. The editor never creates from blank — it only refines something already drafted.
...
> Two exported server actions, `publishPostDirect` and `schedulePost`, also reach
> `deliverPost` but have **no callers anywhere** — no route, no component. They are a
> publish path with nothing driving it. Treat them as dormant, not as a fifth door.
...
| **/social/publish** | Approve · Schedule · Publish | The **review queue**: the next answered full draft awaiting approval, every upcoming one browsable, an editor that fine-tunes copy, attachments and channels — and never creates from blank. Approve (now, or scheduled per the settings), Dismiss, or send the signed review link. An empty queue points back at Draft |
