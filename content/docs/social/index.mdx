...
| **Draft** | `draft` skill — **Claude, natively** — or the **Social Agent window** on `/social` | Arabic-first bilingual copy against [Brand & Voice](/docs/brand). The window drafts server-side via the API — the one decided spend lane (D-20260730), contributor-gated |
...
- **Ledger** — `/social` renders the last 20 variants server-side (brand, channel, copy, status,
  ...
  without SQL.
- **The Social Agent window** (2026-07-30) — describe the post on `/social` and Claude drafts it
  server-side, Arabic first with its English sibling; one press drops the copy into the composer.
  Any contributor, no Claude Code session. This is not the old gateway-LLM draft card coming back:
  that card was removed (2026-07-23) because a *relay* was writing copy — the window calls the
  same Claude that the `draft` skill uses, through the one decided per-draft spend lane
  (`.claude/memory/decisions/2026-07-30-in-app-draft-spend.md`); the cron's no-spend default is
  untouched.
