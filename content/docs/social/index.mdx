...
### The Hub is this table, as tabs

`/social` mirrors the pipeline: seven stages across five tabs, in order, with brand · channels ·
status above them as context that spans all of them.

| Tab | Stages | What is there |
|-----|--------|---------------|
| **Calendar** | Plan | Nothing to operate — the calendar's output is a dated table the Hub has nowhere to keep. The tab names the keyword instead of faking a surface |
| **Draft** | Draft | The Social Agent window. Real |
| **Media** | Media | Nothing to operate — `/higgs` renders to the CDN. The tab says to bring the URL to Publish |
| **Publish** | Approve · Schedule · Publish | The composer. All three are one form with three buttons, so they are one tab rather than three over the same fields |
| **Measure** | Measure | The activity ledger — last 20 variants with status, time, and latest reach/views |

Panels are hidden, never unmounted: the agent polls its queue for minutes and the composer holds
typed copy, and a tab switch must not throw either away. Taking a draft ("Use Arabic") moves the
Hub to **Publish** — the composer is behind another tab, so handing it copy is a stage change.
...
| Who | Where | Does |
|-----|-------|------|
| Any contributor | `/social` | Picks the brand, writes a brief, presses send. The ask becomes a `SocialDraftRequest` and the window polls |
...
```bash
node scripts/social-drafts.mjs list                                    # what is waiting
node scripts/social-drafts.mjs answer <id> --ar ar.txt --en en.txt     # the copy
...
Nothing drains the queue on a schedule. Until something does, an ask waits for the next
session, and the window polls without a timeout — see [Current status](#current-status).
