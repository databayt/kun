...
```
 idea ──► draft ──► media ──► approve ──► schedule ──► publish ──► measure
  │        │         │          │           │            │           │
content   Claude    /higgs    human       queue        Hermes     PostHog
calendar  native   text-free   gate    (phase 2)      gateway     + UTM
```
...
| **Publish** | **Hermes** gateway ([hermes-agent](/docs/hermes)) | Egress only — a dumb relay, never the LLM |
...
| Reach the platform | Hermes gateway ([wiring](/docs/hermes)) | The one thing Claude can't do natively — Slack today via `SLACK_HOME_CHANNEL`; other channels as adapters land |
| Prove it worked | PostHog + UTM | Attribution back to site traffic / conversions |

> **Planned rename.** The current WIP action `generateAndPublishPost` drafts via the gateway's own
...
- **Works** — bilingual (en/ar, RTL) dashboard, channel toggles, a Hermes connection
  health check, and a draft flow (prompt → draft → load into composer).
- **Stubbed** — the *actual posting* lives outside this repo in the Hermes gateway (Slack is
  wired via `SLACK_HOME_CHANNEL`; the other channels await adapters — see [Hermes](/docs/hermes)).
  No persistence, no scheduling, no analytics. Only 4 of the 8 channels are wired in the UI.
  The auth is presence-only and the actions lack Zod validation.
