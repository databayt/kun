...
The Social Hub at [`/engine/social`](/engine/social) is hardened (Phase 2, 2026-07-10):

- **Works** — bilingual (en/ar, RTL) composer with a config-driven channel registry (9 channels;
  **Telegram live via direct Bot API + Slack via Hermes**, the rest marked *soon* until their
  adapters land), per-transport health checks and publish gating,
  Zod-validated publish, a contributor re-check inside the action (JWT sessions outlive allowlist
  removal), and a server-boundary auth guard on the page.
...
- **Still pending** — adapters beyond Slack (see [Hermes](/docs/hermes)), persistence
  (`SocialPost`), scheduling, and metrics pull-back — tracked in `docs/SOCIAL-AUTOMATION.md`.
