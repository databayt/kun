...
|-------|--------|---------------|
| **/social/calendar** | Plan | Nothing to operate — the calendar's output is a dated table the Hub has nowhere to keep. The page names the keyword instead of faking a surface |
| **/social/draft** | Draft | The Social Agent window. Real |
| **/social/media** | Media | The showroom — generated assets (`library.json`) and kept references (`references.json`), filterable by brand and type; decks link to their live renders. Copy URL feeds the Publish stage's Media field |
| **/social/publish** | Approve · Schedule · Publish | The composer. All three are one form with three buttons, so they are one route rather than three over the same fields |
...
agent's queue poll runs on for minutes and the composer keeps its typed copy, and switching
stages must not throw either away. Taking a draft ("Use Arabic") moves the Hub to **Publish** —
the composer is another route, so handing it copy is a navigation.
