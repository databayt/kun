...
> subscription-only (Claude Max, no API-key spend), so flipping the draft source to `anthropic` —
> or setting `SOCIAL_DRAFT_FALLBACK` — needs `/decide` + Abdout's sign-off first. The key sits in
> production, but nothing pulls the trigger while `SOCIAL_AUTOPOST_PRODUCTS` is empty and the
> source defaults to `hermes`. The no-spend path: draft in a Claude Code session (`/draft`, Max
> pool) and paste into the composer.
