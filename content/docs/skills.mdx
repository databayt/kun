---
title: Skills
description: 61 skills across two plugins — 50 in kun-company, 15 in kun-stack (68 user skills)
---

# Skills

A skill is a packaged workflow: one word in, a complete multi-step job out. The engine ships **61 unique skills** across two plugins — 50 in `kun-company`, 15 in `kun-stack` (4 shared). The full user fleet contains 68 skills.
...
| `deploy` | Production deploy — the full Cloudflare cycle (gate, env, schema gap, build, smoke, deploy, restore point, real-login verify); Vercel preview lane kept for legacy repos |
...
| `pwa` | Installable, offline, push — audit a URL, build the per-tenant manifest + service worker + outbox, wire Web Push ([PWA](/docs/pwa)) |
...
| `record` | Screen-record demo videos and screenshot sets of real product flows, filed to media library |
| `higgs` | Generate marketing images and video via Higgsfield |
| `convert` | Any file or URL → clean Markdown |
| `md` | The Markdown standard both converters are held to — graded by agreement between two reads, never by volume |
| `textbook` | A curriculum `textbook.pdf` → a verified Markdown twin: transcribe, blind re-read, adjudicate, benchmark — see [Textbook](/docs/textbook) |
...
| `bench` | Score the engine's own config on labeled cases, propose holdout-gated fixes |
| `crawl-anthropic` | Crawl and snapshot Anthropic documentation and assets |
